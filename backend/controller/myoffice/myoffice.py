...
users_api = db.users.Users()
my_office_api = db.myoffice.My_Office()
...
class My_Office:
    ...
    def update_slogen_by_name(name, slogen):
        try:
            is_updated = my_office_api.update_slogen_by_store(slogen, name)
            print(slogen)
            if is_updated:
    ...
    def update_bunner_by_name(name, bunner):
        try:
            is_updated = my_office_api.update_bunner_by_store(bunner, name)
            if is_updated:
    ...
    def update_profile_by_name(name, profile):
        try:
            is_updated = my_office_api.update_profile_by_store(profile, name)
            if is_updated:
    ...
    def update_phone_by_name(name, phone):
        try:
            is_updated = my_office_api.update_phone_by_store(phone, name)
            print(phone)
            if is_updated:
    ...
    def update_email_by_name(name, email):
        try:
            is_updated = my_office_api.update_email_by_store(email, name)
            print(email)
            if is_updated:
    ...
    def update_links_by_name(name, links):
        try:
            is_updated = my_office_api.update_links_by_store(links, name)
            print(links)
            if is_updated:
    ...
    def update_description_by_name(name, description):
        try:
            is_updated = my_office_api.update_description_by_store(description, name)
            if is_updated:
    ...
    def get_store_by_storeusername(name):
        try:
            store = my_office_api.get_store_by_name(name)
            print(store)
            if store == None:
    ...
    def get_store_by_username(name, user):
        try:
            ...
            print(decoded_store)
            print(decoded_user)
            get_store = my_office_api.get_store_by_id(decoded_store["store_id"])
    ...
    def get_store_by_id3(id):
        try:
            result = my_office_api.get_store_by_id(id)
    ...
    def set_new_store(id, name, email):
        try:
            ...
            result = my_office_api.insert_store(store)
            username = users_api.get_user_by_email(email)
            if result:
                get_Token = my_office_api.get_store_by_name(name)
