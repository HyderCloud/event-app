...
const Home = () => {
  ...
  useEffect(() => {
    if (search.get("isbuissness") === 'true') {
      onOpen()
    }
  }, [])
  return (
    <div className='w-full min-full dashboard-container flex flex-col' style={{ paddingTop: '5%', gap: '25px' }}>
      <Modal isOpen={isOpen} onOpenChange={onOpenChange}>
        <ModalContent>
          {(onClose) => (
            <>
              <ModalHeader className="flex flex-col gap-1">  האם אתה רוצה משתמש עסקי ב-Eventrise?</ModalHeader>
              <ModalBody className="flex flex-col">
              <div>
              מערכת <strong>Eventrise</strong> משלבת את כל הכלים שאתה צריך לניהול עסק – CRM ו-SRM תחת גג אחד.
            אם תבחר לפתוח משתמש עסקי, תוכל לנהל את הספקים והלקוחות שלך בצורה הרבה יותר יעילה, לגשת לפיצ'רים
            מתקדמים של פרסום, וליהנות ממערכת ניהול מותאמת אישית שתפנה לך זמן להתרכז בעסק.
              </div>
              <div>
              האם תרצה לשדרג את החשבון שלך לחשבון עסקי?
              </div>
              </ModalBody>
              <ModalFooter>
              <Button auto color="success" onClick={() => router.push('/buisness')}>
            כן, אני רוצה משתמש עסקי
          </Button>
          <Button auto color="danger" onClick={()=>{onClose()
            router.push('/')
          }}>
            לא, תודה
          </Button>
