...
const changeGrisdByIndex = (arr)=>{
  ...
  arr1[6] = {   headerName: "הצעות מחיר",
    cellRenderer: quatationColumn,}
  return arr1
}
const quatationColumn = ({data})=>{
  return(
    <div className='flex justify-center items-center'><Button onPress={()=>{setPreviewReq(data.quotations)
      onOpen2()
...
const statusColumn = ({data})=>{
  ...
  return(
    <div style={{backgroundColor: data.status === 'בהמתנה'? '#71717A': "#34a853",color: 'white',textAlign: 'center'}}>{data.status}</div>
  )
}
