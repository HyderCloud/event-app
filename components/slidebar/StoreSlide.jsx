...
const StoreSlide = () => {
    ...
    if (pathName.split('/')[1] === 'dashbord' || pathName === '/buisness') {
        ...
        return (
            <div className='flex flex-col flex-wrap gap-4 w-full h-screen fixed items-center justify-center '
                style={{ width: '80px', backgroundColor: '#F8FAFC', gap: '20px', overflowY: 'hidden' }}>
                <div className="flex h-full items-center justify-center flex-wrap gap-10">
                    ...
                    >
                        ...
                        {storeType === "buisness" && <Tab key='myoffice' href={`myoffice/${decodedToken?.name}`}
                        ...
                        <Tab key='events' href={`/events/${decodedToken?.name}`} title={<div className='w-full h-full flex justify-center items-center'
                            style={{
                                background: (pathName === `/events/${decodedToken?.name}` || isHover === 'events') && '#aad28e'
